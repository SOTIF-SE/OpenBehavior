...
class JudgeByDis:
    ...
    def init_trace_data(self):
        # npc_name_list = self.get_npc_name()
        # npc_num = len(npc_name_list)
        # for npc_name in npc_name_list:
        #     self.trace_data[npc_name] = []
        ...
        for j in range(self.npc_num):
            ...
            self.final_location[self.json_data["npcList"][j]["ID"]] = self.json_data["npcList"][j]["motion"][len(self.json_data["npcList"][j]["motion"]) - 1]["location"]
        for l in range(0, self.timestamp_npc):
    ...
    def get_target_location(self):
        file = "/home/abc/scenario_runner_able_edition/ego_info/ego_des.json"
        with open(file, "r") as f:
            data = json.load(f)
        self.target_location = data

    def init_ego_trace_data(self):
        for l in range(0, self.timestamp_npc):
    ...
    def compute_min_between(self):
        ...
        for npc in self.npc_name:
            min_score = float('inf')
            for t in range(self.timestamp_npc):
    ...
    def com_final_location(self):
        fin_ego_location = self.ego_trace_date[self.timestamp_npc - 1]
        yaw = self.json_data["trace"][self.timestamp_npc - 1]["ego"]["pose"]["rotation"]["yaw"]
        ...
        for i in self.final_location:
            ...
            if 0 < forward_dist < 5 and 0 < forward_dist <= destination_dis and abs(lateral_dist) < 2.0:
                return True
        return False
    ...
    def get_is_collision(self):
        ...
        for i in range(self.timestamp_npc):
            ...
            if self.min_dist_to_ego[i] < 3.0 and self.is_true_collision(min_distance_ego_location,
                                                                        min_distance_npc_location,
                                                                        min_distance_ego_yaw) >= 0:
                self.is_collision = True
                break

    def remove_avi(self):
        fileidx = 0
        while os.path.exists("/home/abc/scenario_runner_able_edition/Law_Judgement/traffic_accident_video/accident_{}.avi".format(fileidx)):
            fileidx += 1
        video_path = "/home/abc/scenario_runner_able_edition/Law_Judgement/traffic_accident_video/accident_{}.avi".format(fileidx - 1)
        if self.is_collision and self.scoreB <= -12:
            print("It is good")
        else:
    ...
    def compute_max_to(self):
        ...
        if self.com_final_location():
            dis = self.max_to_target - compute_distance(self.target_location, self.ego_trace_date[self.timestamp_npc - 1])
        else:
            # 前方无NPC
            dis = self.max_to_target - compute_distance(self.target_location,
                                                        self.ego_trace_date[self.timestamp_npc - 1]) + 8.0
        self.scoreB = dis
    ...
    def compute_score(self):
        ...
        if self.scoreB == -12 and not self.is_collision:
            return float('-inf')
        return min(self.scoreA, self.scoreB)
...
class EncodedTestCase:
    ...
    def compute_robustness(self):
        judge = JudgeByDis(self.trace)
        value = judge.compute_score()
        self.fitness = value * (-1)
...
def generate_trace(data, runtime=False, session="scenario"):
    ...
    while attempts_conter < 1 and not os.path.isfile(trace_temp_file):
        print(attempts_conter)
        # CarlaExists = False
        ...
        #     time.sleep(15)
        try:
            start_docker.start_pro(data_temp_file)
            # bashCommand = "conda run -n scen python3 ../scenario_runner.py --sync --reloadWorld --waitForEgo --able " + data_temp_file
            start_run = time.perf_counter()
            # process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
            # output, error = process.communicate()
            subprocess.run(
                ["conda", "run", "-n", "scen", "python3", "../scenario_runner.py", "--sync", "--waitForEgo", "--able",
                 data_temp_file])
            ...
            print("Stop docker and prepare for next scenario!")
            # 终止全部docker进程
...
if __name__ == "__main__":
    ...
    if os.path.isdir(population_path_initial + sub_population_path_initial):
        ...
        for initial_testcase in initial_testcase_list:
            ...
            # with open(population_path_generation + sub_population_path_initial + "/generation_" + str(
            #         generation_counter) + "/" + initial_testcase) as f:
            #     mutated_testcase_list = json.load(f)
            ...
            while generation_counter < MAX_GENERATION:
                ...
                # with open(population_path_generation + sub_population_path_initial + "/generation_" + str(generation_counter) + "/traceset_0_avunit_s4.json") as f:
                #     dataset = json.load(f)
                ...
                total_time = total_gen_time + gen_time
                ...
                if not os.path.isdir(population_path_generation + sub_population_path_initial + "/generation_" + str(generation_counter)):
                    os.mkdir(population_path_generation + sub_population_path_initial + "/generation_" + str(generation_counter))
                with open(population_path_generation + sub_population_path_initial + "/generation_" + str(generation_counter) + "/" + initial_testcase,
                          "w", encoding="utf-8") as f:
                    json.dump(mutated_testcase_list, f)

                print(generation_counter, "th generation of mutated_" + initial_testcase + " is generated.")

                record_time_path = "/home/abc/scenario_runner_able_edition/time_record/"+sub_population_path_initial+".json"
                average_gen_scenario_time = total_gen_time / (generation_counter * 20)
                average_run_scenario_time = total_run_time / (generation_counter * 20)
                record_time = {
                    "average_gen_scenario_time": average_gen_scenario_time,
                    "average_run_scenario_time": average_run_scenario_time
                }
                save_result(record_time, record_time_path)
