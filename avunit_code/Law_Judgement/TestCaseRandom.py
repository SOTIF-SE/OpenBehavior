...
if __name__ == "__main__":
    ...
    if os.path.isdir(initial_path + initial_subpath):
        for trace in os.listdir(initial_path + initial_subpath):
            ...
            if not "temp" in trace and not os.path.isfile(
                    randomized_path + initial_subpath + "/" + trace.replace("trace", "traceset_" + str(0))):
                testcase = TestCaseRandom(data)
                testcase.testcase_random(20)
                with open(randomized_path + initial_subpath + "/" + trace.replace("trace", "traceset_" + str(0)),
                          "w") as traceset_file:
                    json.dump(testcase.cases, traceset_file, indent=4)
