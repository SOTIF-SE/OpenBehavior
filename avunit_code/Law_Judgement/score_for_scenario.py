...
Record_Path = "/home/abc/scenario_runner_able_edition/scenario_record/avunit_s4.json"
...
def log_data(data, filename="/home/abc/scenario_runner_able_edition/log.txt"):
    with open(filename, "a", encoding="utf-8") as f:
        f.write(data + "\n")

def log_special_data(data, filename="/home/abc/scenario_runner_able_edition/special_log.txt"):
    with open(filename, "a", encoding="utf-8") as f:
        f.write(data + "\n")

class ScoreForScenario:
    def __init__(self, json_path, gen=0, idx=0):
        self.path = json_path
        ...
        # self.monitor = Monitor(self.json_data)
        # self.law_result = self.monitor.continuous_monitor_for_violations()
    ...
    def compute_lane_change_by_is_lane_changing(self):
        ...
        for i in range(self.timestamp):
            if self.json_data["trace"][i]["ego"]["isLaneChanging"] and flag == 0:
                ...
                flag = 0
        self.scoreC = self.change_lane_times - 2
    ...
    def compute_score(self):
        ...
        self.value = testtask_rob * 0.3 + designtask_score * 0.7
...
if __name__ == "__main__":
    file = "/home/abc/scenario_runner_able_edition/Law_Judgement/trace/temp_avunit_s4"
    acc_idx = 0
    for filename in sorted(os.listdir(file)):
        # if filename == "trace_avunit_s1_0_-1.json":
        #     continue
        # match = re.search(r"trace_avunit_s1_(\d+)_(\d+)\.json", filename)
        ...
        if filename.endswith(".json"):
            ...
            record_s = ScoreForScenario(file_path)
            testtask_rob = min(record_s.scoreA, record_s.scoreB) * (-1)
            designtask_score = record_s.scoreC
            if record_s.is_collision or record_s.scoreB < -6:
                acc_idx += 1
                print(acc_idx)
                print(record_s.is_collision, record_s.scoreA, record_s.scoreB, testtask_rob, record_s.scoreC, testtask_rob * 0.3 + designtask_score * 0.7)
