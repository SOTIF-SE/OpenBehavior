...
class LaneModifier(Modifier):
    ...
    def get_lane_id(self):
        lane_id = self.args.get('lane')
        if lane_id.startswith('[') and lane_id.endswith(']'):
            lane_id = lane_id.strip("[]")
            val_start = lane_id.split("..")[0]
            val_end = lane_id.split("..")[1]
            lane_id = random.randint(int(val_start), int(val_end))
            return lane_id
