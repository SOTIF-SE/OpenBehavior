...
class ABLEScenario(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        """
        root = py_trees.composites.Parallel("Root", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        timeout = TimeOut(80, name="duration=" + str(80))
        root.add_child(timeout)

        # EGO，用于场景完全复现，完全轨迹复刻
        # 正常使用需要删除
        # ego = self.ego_vehicles[0]
        # ego_behavior = py_trees.composites.Sequence("Ego", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL)
        # ego_start_location = carla.Location(x=self.data["trace"][0]["ego"]["pose"]["position"]["x"],
        #                                     y=self.data["trace"][0]["ego"]["pose"]["position"]["y"],
        #                                     z=self.data["trace"][0]["ego"]["pose"]["position"]["z"])
        # ego_start_wp = self.map.get_waypoint(ego_start_location)
        # ego_behavior.add_child(ActorTransformSetter(ego, ego_start_wp.transform))
        #
        # flag = 0
        # for idx in range(len(self.data["trace"])):
        #     if idx == 0:
        #         continue
        #     previous_location = carla.Location(x=self.data["trace"][flag]["ego"]["pose"]["position"]["x"],
        #                                        y=self.data["trace"][flag]["ego"]["pose"]["position"]["y"],
        #                                        z=self.data["trace"][flag]["ego"]["pose"]["position"]["z"])
        #     previous_waypoint = self.map.get_waypoint(previous_location)
        #
        #     location = carla.Location(x=self.data["trace"][idx]["ego"]["pose"]["position"]["x"],
        #                               y=self.data["trace"][idx]["ego"]["pose"]["position"]["y"],
        #                               z=self.data["trace"][idx]["ego"]["pose"]["position"]["z"])
        #     waypoint = self.map.get_waypoint(location)
        #
        #     if location.distance(previous_location) < 8:
        #         continue
        #     flag = idx
        #
        #     # Set Speed
        #     if idx < len(self.data["trace"]) - 1:
        #         # target_speed = (npc["motion"][idx]['speed'] + npc["motion"][idx+1]['speed'])/2
        #         target_speed = max(self.data["trace"][idx]["ego"]["Chasis"]["speed"],
        #                            self.data["trace"][idx+1]["ego"]["Chasis"]["speed"])
        #     else:
        #         # target_speed = (npc["motion"][idx]['speed'] + npc["destination"]['speed'])/2
        #         target_speed = max(self.data["trace"][idx]["ego"]["Chasis"]["speed"],
        #                            self.data["trace"][idx]["ego"]["Chasis"]["speed"])
        ...
        #             WaypointFollower(ego, target_speed=target_speed, plan=route, avoid_collision=False,
        #                              name="FollowWaypointsABLE"))
        # ego_behavior.add_child(StopVehicle(ego, 1.0))
        ...
        for npc in self.data["npcList"]:
            ...
            for idx in range(len(npc["motion"])):
                if idx == 0:
                    continue
                previous_lane_position = npc["motion"][flag]['lane_position']
                ...
                previous_location = previous_waypoint.transform.location
