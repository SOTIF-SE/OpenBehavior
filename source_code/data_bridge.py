...
class DataBridge(object):
    ...
    """
    def __init__(self, world, tag = None):
        """
        ...
        self.trace_list = []
        self.chang_lane_times = {}
        self.first_change_wp = {}
        ...
        if tag is None:
            ...
            match = re.search(r"generation_(\d+)/ind_(\d+)\.json", tag)
            self.gen_value = int(match.group(1))
            self.idx_value = int(match.group(2))
    ...
    def update_ego_vehicle_start(self) -> None:
        ...
        self.chang_lane_times["ego_vehicle"] = 0
        self.first_change_wp["ego_vehicle"] = None
    ...
    def update_npc_vehicle_start(self) -> None:
        for idx in range(len(self.other_actors)):
            ...
            npc_id = "npc" + str(idx)
            self.chang_lane_times[npc_id] = 0
            self.first_change_wp[npc_id] = None
    ...
    def update_npc_vehicle_motion(self) -> None:
        for idx in range(len(self.other_actors)):
            ...
            # if self.isLaneChanging_actor(npc):
            #     if self.first_change_wp["npc"+str(idx)] is None:
            #         wp = self._map.get_waypoint(
            #             npc_location,
            #             project_to_road=True,
            #             lane_type=carla.LaneType.Driving
            #         )
            #         self.first_change_wp["npc"+str(idx)] = wp
            #     right_vec = self.first_change_wp["npc"+str(idx)].transform.get_right_vector()
            #     rel = npc_location - self.first_change_wp["npc"+str(idx)].transform.location
            #     lateral = rel.x  * right_vec.x + rel.y * right_vec.y
            #     if abs(lateral) > self.first_change_wp["npc"+str(idx)].lane_width * 0.8:
            #         self.chang_lane_times["npc"+str(idx)] += 1
            #         self.first_change_wp["npc" + str(idx)] = self._map.get_waypoint(
            #             npc_location,
            #             project_to_road=True,
            #             lane_type=carla.LaneType.Driving
            #         )
            # else:
            #     self.first_change_wp["npc"+str(idx)] = None
            ...
            self.trace['npcList'][idx]['motion'].append({
                ...
                "yaw": npc_transform.rotation.yaw,
                "lane_position": {
                    "lane": "lane_" + str(npc_wp.road_id),
                    "offset": npc_wp.s,
                    "roadID": npc_wp.lane_id
                ...
                "isLaneChanging": self.isLaneChanging_actor(npc),
                "speed": calculate_velocity(npc),
    ...
    def end_trace(self, path, scenario) -> None:
        ...
        des_location = self.ego_vehicle.get_location()
        ...
        with open(path + scenario +'/{}_{}_{}_{}.json'.format(filetype, scenario, self.gen_value, self.idx_value), 'w', encoding='utf-8') as fw:
    ...
    def update_trace(self) -> None:
        ...
        #
        # if self.isLaneChanging():
        #     if self.first_change_wp["ego_vehicle"] is None:
        #         wp = self._map.get_waypoint(
        #             ego_location,
        #             project_to_road=True,
        #             lane_type=carla.LaneType.Driving
        #         )
        #         self.first_change_wp["ego_vehicle"] = wp
        #     right_vec = self.first_change_wp["ego_vehicle"].transform.get_right_vector()
        #     rel = ego_location - self.first_change_wp["ego_vehicle"].transform.location
        #     lateral = rel.x * right_vec.x + rel.y * right_vec.y
        #     if abs(lateral) > self.first_change_wp["ego_vehicle"].lane_width*0.8:
        #         self.chang_lane_times["ego_vehicle"] += 1
        #         self.first_change_wp["ego_vehicle"] = self._map.get_waypoint(
        #             ego_location,
        #             project_to_road=True,
        #             lane_type=carla.LaneType.Driving
        #         )
        ...
        frame_dict['truth'] = {
            ...
            "NearestNPCSpeed": self.getNpcSpeed(self.getNearestNPC()),
            "NearestNPCLocation": self.getNpcLocation(self.getNearestNPC()),
    ...
    def isLaneChanging_actor(self, actor):
        actor_transform = actor.get_transform()
        road_wp = self._map.get_waypoint(actor_transform.location, project_to_road=True)
        distance_to_lane = actor_transform.location.distance(road_wp.transform.location)

        actor_length = actor.bounding_box.extent.x
        actor_width = actor.bounding_box.extent.y
        actor_theta = math.radians(actor.bounding_box.rotation.yaw - road_wp.transform.rotation.yaw)

        return distance_to_lane + actor_length*abs(math.sin(actor_theta)) + actor_width*abs(math.cos(actor_theta)) > road_wp.lane_width
