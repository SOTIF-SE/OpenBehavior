...
class ScenarioRunner(object):
            ...
            while ego_vehicle_missing:
                ...
                for ego_vehicle in ego_vehicles:
                    ...
                    if not ego_vehicle_found:
                        ego_vehicle_missing = True
                        break
            data = self.get_ego_info_from_data()
            world = CarlaDataProvider.get_world()
            lane_position_destination = data["ego"]["start"]['lane_position']
            tr = world.get_map().get_waypoint_xodr(
                int(lane_position_destination['lane'].replace("lane_", "")),
                lane_position_destination['roadID'],
                lane_position_destination['offset']
            ).transform
            for i, _ in enumerate(self.ego_vehicles):
                self.ego_vehicles[i].set_transform(tr)
    ...
    def _load_and_wait_for_world(self, town, ego_vehicles=None):
        """
        Load a new CARLA world and provide experiment_result to CarlaDataProvider
        """
        ...
        if self._args.sync:
            ...
            settings.fixed_delta_seconds = 1.0 / 40.0
            self.world.apply_settings(settings)
    ...
    def _load_and_run_scenario(self, config):
        ...
        try:
            ...
            if self._args.openscenario:
                ...
                scenario = OSC2Scenario(world=self.world,
                                        ego_vehicles=self.ego_vehicles,
                                        config=config,
                                        osc2_file=self._args.openbehavior,
                                        ga_mode=self._args.GA,
                                        mutation_json=self._args.config_json,
                                        timeout=100000)
            else:
        ...
        try:
            ...
            self.manager.data_bridge = DataBridge(self.world, self._args.config_json)
            self.manager.load_scenario(scenario, self.agent_instance, self._args.config_json)
            self.end_time = time.time()
            self.start_scenario_time = self.end_time - self.start_time

            if os.path.exists("/home/abc/scenario_runner/scenario_record/obehavior_s4_extra_time.json"):
                time_data = load_template()
            else:
                time_data = {}
            time_data["average_start_scenario_time"] = ((time_data.get("average_start_scenario_time", 0) * time_data.get("scenario_num", 0)
                                                         + self.start_scenario_time)/ (time_data.get("scenario_num", 0) + 1))
            save_result(time_data)

            self.manager.run_scenario()

            # if self._args.waitForEgo and self._args.GA and self._args.config_json is not None:
    ...
    def _run_ob(self):
        if not os.path.isfile(self._args.openbehavior):
            print("File does not exist")
            self._cleanup()
            return False

        config = OSC2ScenarioConfiguration(self._args.openbehavior, self.client)
...
def main():
    ...
    parser.add_argument('--openbehavior', help='Provide an openbehavior definition')
    parser.add_argument(
    ...
    if not arguments.scenario and not arguments.openscenario and not arguments.route and not arguments.openscenario2 and not arguments.openbehavior:
