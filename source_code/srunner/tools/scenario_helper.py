...
def generate_target_waypoint_list_samelane(waypoint, distance_same_lane = 50,step_distance = 2):
    ...
    if distance < distance_same_lane:
        if  plan[-1][0].get_left_lane():
            plan.append((plan[-1][0].get_left_lane().next(step_distance), RoadOption.LANEFOLLOW))
            while distance < distance_same_lane:
                next_wps = plan[-1][0].next(step_distance)
                if not next_wps:
                    break
                ...
                distance += next_wp.transform.location.distance(plan[-1][0].transform.location)
                plan.append((next_wp, RoadOption.LANEFOLLOW))
        elif  plan[-1][0].get_right_lane():
            plan.append((plan[-1][0].get_right_lane().next(step_distance), RoadOption.LANEFOLLOW))
            while distance < distance_same_lane:
                next_wps = plan[-1][0].next(step_distance)
                if not next_wps:
                    break
