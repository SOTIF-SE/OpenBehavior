...
def reset_config(new_info, path_to_json):
...
def process_speed_modifier_with_para(
        config, modifiers, duration: float, all_duration: float, father_tree, config_json, path_to_config
):
    if not modifiers:
        return
    ...
    global ego_lane
    global car_info
    global npc1_start_wp
    global npc1_start_position
        ...
        if isinstance(modifier, SetBMModifier):
            ...
            if agent_type == 'AI':
                ...
                if bm_name == "internal_npc":
                    ...
                    set_bm = IniBM(actor, bm_name)
                    father_tree.add_child(set_bm)
                elif bm_name == "sudden_brake_agent":
                    is_model[actor_name] = True
                    set_bm = IniBM(actor, bm_name)
                    father_tree.add_child(set_bm)
            ...
            ego_lane = para_data['ego_vehicle']["start_position"]['lane_id']
            lane_change = None
            speed_value = None
            ...
            if actor_name != "ego_vehicle":
                ...
                if start_distance is not None:
                    ...
                    start_wp = wp_lists[0]
                    start_position = start_wp.transform.location
                    # print(actor_name, start_position)
                    # if start_distance < 0:
                ...
                lane_position = para_data[actor_name]["start_position"]["lane_position"]
                wp = CarlaDataProvider.get_world().get_map().get_waypoint_xodr(
                ...
                if is_model[actor_name]:
                    # distance = 1.0
                    set_behavior_logic = SetBehaviorLogic(actor, start_position, distance, start_lane, end_lane, is_confirm=False)
                    father_tree.add_child(set_behavior_logic)
                else:
                    # # Get the global route planner, used to calculate the route
                    # dao = GlobalRoutePlannerDAO(CarlaDataProvider.get_world().get_map(), 0.5)
            ...
            if not para_data[actor_name]['change_mode']:
                actor = CarlaDataProvider.get_actor_by_name(actor_name)
                ...
                new_info[actor_name] = {
                    ...
                    "range":{
                        "range_lane_start": [
                            para_data[actor_name]['range']['range_lane_start']
                        ],
                        "range_lane_end": [
                            para_data[actor_name]['range']['range_lane_end']
                        ],
                        "range_position_start": [
                            para_data[actor_name]['range']['range_position_start']
                        ],
                        "range_position_end": [
                            para_data[actor_name]['range']['range_position_end']
                        ]
                    },
                    "orch": True,
                    "change_mode": para_data[actor_name]['change_mode']
                }
            else:
                ...
                new_info[actor_name] = {
                    ...
                    "range":{
                        "range_lane_start": [
                            logic['range_lane_start_bot'],
                            logic['range_lane_start_top']
                        ],
                        "range_lane_end": [
                            logic['range_lane_end_bot'],
                            logic['range_lane_end_top']
                        ],
                        "range_position_start": [
                            logic['range_position_start_bot'],
                            logic['range_position_start_top']
                        ],
                        "range_position_end": [
                            logic['range_position_end_bot'],
                            logic['range_position_end_top']
                        ]
                    },
                    "orch": True,
                    "change_mode": False
                }
            reset_config(new_info, path_to_config)
            # ego_distance = ego_speed * duration
            if behavior_type == "AI":
                is_model[actor_name] = True
                max_speed = hyperparameters.get('max_speed', 10)
...
def record_info(car_info):
    ...
    for name in car_info:
        if name == "scenario_mode":
            scenario_config.scenario_mode_w(car_info[name])
    ...
    for modifier in modifiers:
        ...
        if isinstance(modifier, SpeedModifier):
            ...
            if actor_name != "ego_vehicle":
                ...
                car_info[actor_name]["range_lane_start"] = [lane_start_bot, lane_start_top]
                car_info[actor_name]["range_lane_end"] = [lane_end_bot, lane_end_top]
            ...
            car_info[actor_name]['range_lane_start'] = [logic['range_lane_start_bot'], logic['range_lane_start_top']]
            car_info[actor_name]['range_lane_end'] = [logic['range_lane_end_bot'], logic['range_lane_end_top']]
            car_info[actor_name]['range_position_start'] = [logic['range_position_start_bot'], logic['range_position_start_top']]
            car_info[actor_name]['range_position_end'] = [logic['range_position_end_bot'],logic['range_position_end_top']]
            car_info[actor_name]['orch'] = True
            if start_location is not None:
...
class OSC2Scenario(BasicScenario):
    ...
    def __init__(
        ...
        if ga_mode:
            with open(self.mutation_json, "r") as f:
    ...
    class BehaviorInit(ASTVisitor):
        ...
        def visit_do_member(self, node: ast_node.DoMember):
            ...
            if sub_node is None:
                ...
                if not self.father_ins.GA_mode:
        ...
        def visit_choose_directive(self, node: ast_node.chooseDirective):
            ...
            if self.father_ins.GA_mode:
                rand_child = self.father_ins.config_json["scenario_mode"]["now_num"]
            else:
                car_info["scenario_mode"]["choose_num"] = child_num
                car_info["scenario_mode"]["now_num"] = rand_child
        ...
        def visit_judge_exp(self, node: ast_node.judgeExp):
            ...
            for child in node.get_children():
                if isinstance(child, ast_node.judgeDeclaration):
                    ...
                    if self.father_ins.GA_mode:
                        car_info["scenario_mode"]["mode"] = self.father_ins.config_json["scenario_mode"]["mode"]
                        car_info["scenario_mode"]["choose_num"] = self.father_ins.config_json["scenario_mode"]["choose_num"]
                        car_info["scenario_mode"]["now_num"] = self.father_ins.config_json["scenario_mode"]["now_num"]
                    else:
                        car_info["scenario_mode"]["mode"] = mode
                        car_info["scenario_mode"]["choose_num"] = 1
                        car_info["scenario_mode"]["now_num"] = 0
