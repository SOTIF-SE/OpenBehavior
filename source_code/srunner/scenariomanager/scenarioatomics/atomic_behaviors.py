...
# from Roach.eval_agent import EvalAgent
...
class SetBehaviorLogic(AtomicBehavior):
    ...
    set an agent that control the vehicle
    """
    def __init__(self, actor, start_position, end_distance=0, start_lane=1, end_lane=1, global_priority=False,
                 is_confirm=False, name="Set_Behavior_Logic"):
        ...
        self.lane_diff = start_lane - end_lane
        if self.lane_diff > 0:
            self.change = 'left'
        elif self.lane_diff < 0:
            self.change = 'right'
    ...
    def initialise(self):
        ...
        self._ego_vehicle = CarlaDataProvider.get_actor_by_name('ego_vehicle')
        ...
        if not self.is_confirm:
            ...
            if self.change:
                if self.change == 'left':
                    ...
                    for i in range(abs(self.lane_diff)):
                        _end_wp = _end_wp.get_left_lane()
                else:
                    _end_wp = _start_wp.next(self._end_position)[0]
                    for i in range(abs(self.lane_diff)):
                        _end_wp = _end_wp.get_right_lane()
            ...
            _end_position = CarlaDataProvider.get_endwaypoint_by_actorname_and_laneid(self._actor.attributes["role_name"])
            self.end_position = _end_position
        # _start_wp = map.get_waypoint(_start_position)
        #
        # if self.change:
        ...
        #     _end_position = _end_wp.transform.location
        # self.end_position = _end_position
        if self._target_agent == "normal":
            self._agent = BehaviorAgent(self._actor, behavior="normal")
            self._agent.set_destination(_end_position, _start_position)

        elif self._target_agent == "cautious":
            self._agent = BehaviorAgent(self._actor, behavior="cautious")
            self._agent.set_destination(_end_position, _start_position)

        elif self._target_agent == "aggressive":
            self._agent = BehaviorAgent(self._actor, behavior="aggressive")
            self._agent.set_destination(_end_position, _start_position)

        elif self._target_agent == "adversarial_agent":
            self._agent = CuttingInAgent(self._actor, _end_position, _start_position)
            self._agent.set_target_speed(15)

        elif self._target_agent == "sudden_brake_agent":
            self._agent = SuddenBrakeAgent(self._actor, _end_position, _start_position)
            self._agent.set_target_speed(15)
            ...
            self.agent = LAVAgent("/home/abc/scenario_runner/config.yaml")
        ...
        #     gps_route, self.route = interpolate_trajectory(self._world, [_start_position, _end_position],
        #                                                    hop_resolution=1.0)
        #     self.agent = EvalAgent("/home/lhy/projects/scenario_runner/Roach/model/Roach_00/")
        #     self.agent.set_global_plan(gps_route, self.route)
        #     CarlaDataProvider.get_world().tick()
        #
        #     self.agent.sensor_interface = SensorInterface()
    ...
    def update(self):
        new_status = py_trees.common.Status.RUNNING
        if self._target_agent == "cautious" or self._target_agent == "aggressive" or self._target_agent == "normal":
        ...
        if calculate_distance(current_location, self.end_position) < 6.0 and CarlaDataProvider.get_velocity(self._actor) < 1.0:
