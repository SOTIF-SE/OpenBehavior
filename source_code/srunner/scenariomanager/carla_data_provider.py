...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def trans_velocity(ego_speed, rotation):
        # 输入，主车速度， npc车辆车道方向
        # 返回，npc车辆速度
        vx = ego_speed * np.cos(rotation / 180 * np.pi)
        vy = ego_speed * np.sin(rotation / 180 * np.pi)
    ...
    def set_world(world):
        ...
        CarlaDataProvider.junction_s8 = {
            "map": "Town04",
            "junction": None,
            "target_traffic_light_opendrive_id": "2405"
        }
        CarlaDataProvider.actor2roadid_s8_start = {
            "junction": None,
            "ego_vehicle": 38,
            "npc1": 38,
            "npc2": 38,
        ...
        CarlaDataProvider.actor2roadid_s8_end = {
            "junction": None,
            "ego_vehicle": (38, 2),
            "npc1": (38, 2),
            "npc2": (38, 2),
    ...
    def get_waypoint_by_actorname_and_laneid(actor_name: str, lane_id: int):
        ...
        pos = CarlaDataProvider.generate_road_spawn_points(actor2roadid_start[actor_name],
                                                           actor2roadid_start["junction"])[0]
        wp = CarlaDataProvider.get_map().get_waypoint(pos.location, project_to_road=True,
    ...
    def generate_road_spawn_points(road_id, junction_id=None, distance=4.0):
        ...
        for spawn_point in spawn_points:
            if CarlaDataProvider.road_point_is_junction(spawn_point):
                spawn_points.remove(spawn_point)
        return spawn_points

    @staticmethod
    def get_waypoint_by_special_laneid(lane_id: int):
        spawn_point = CarlaDataProvider.generate_road_spawn_points(37)
        pos = spawn_point[7]
        wp = CarlaDataProvider.get_map().get_waypoint(pos.location, project_to_road=True,
                                                      lane_type=carla.LaneType.Driving)
    ...
    def request_new_batch_actors_with_specified_model_sets(
        ...
        for i in range(amount):
            ...
            blueprint = CarlaDataProvider.create_blueprint(
                model,
                rolename + str(traffic_flow_nums + i).zfill(4),
                color=coler,
            ...
            if spawn_point:
                vec_speed = veloc
                # if CarlaDataProvider.get_local_location(ego_actor,spawn_point.location).x < -10:
                #     vec_speed = CarlaDataProvider.get_speed(ego_actor) *2
                if autopilot:
                    batch.append(
                        ...
                        .then(
                            Velocity(
                                FutureActor,
                                CarlaDataProvider.trans_velocity(
                                    vec_speed,
